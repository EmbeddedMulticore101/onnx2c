...
void initialize_logging(void)
{
	...
	switch (options.logging_level) {
		...
		default:
		case 2:
			s = AixLog::Severity::info;
			break;
		case 1:
			s = AixLog::Severity::warning;
			break;
		case 0:
			s = AixLog::Severity::error;
			break;
	}
...
void parse_cmdline_options(int argc, const char* argv[])
{
	...
	args::Flag quant(parser, "quant", "Enable quant feature", {'q', "quant"});
	...
	if (avr) {
		options.target_avr = true;
	}
	if (noGlobals) {
		options.no_globals = true;
	}
	if (externInit) {
		options.extern_init = true;
	}
	if (onlyInit) {
	...
	if (quant) {options.quant = true; }
	if (input) {
		options.input_file = args::get(input);
	}
	if (funcName) {
		options.interface_func_name = args::get(funcName);
	}
